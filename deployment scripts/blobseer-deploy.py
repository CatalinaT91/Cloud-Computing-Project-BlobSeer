...
def gen_launch(path, process):
    ...
    return "\"rm -rf " + out + "; " + \
        ...
        "cd " + get_env("BLOBSEER_HOME") + "/" + path + "/" + "; " + "./" + process + " " + REMOTE_CFG_FILE + " >" + out + "/" + process + ".stdout 2>" + out + "/" + process + ".stderr& \""

def gen_kill(path, process):
    out = "/tmp/blobseer/" + process
    return "\"killall " + process + " || echo could not kill " + process + "; " + \
	"if [ -f socket.log ]; then mv " + get_env("BLOBSEER_HOME") + "/" + path + "/socket.log " + out + "/" + process + "-socket.log; fi\""
